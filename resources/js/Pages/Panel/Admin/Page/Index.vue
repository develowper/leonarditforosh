    ...
    <template v-slot:content>
      ...
      <div
        ...
        <div class="flex">
          ...
          <h1 class="text-2xl font-semibold">{{ __('pages_list') }}</h1>
        </div>
        <div>
          <Link :href="route('panel.admin.page.create')"
            ...
            <tr v-for="(d,idx) in data"
              ...
              <td class="px-2 py-4    " data-te-dropdown-ref>
                <button
                    ...
                    :class="`bg-${getStatus('page', d.status).color}-100 hover:bg-${getStatus('page', d.status).color}-200 text-${getStatus('page', d.status).color}-500`">
                  {{ getStatus('page', d.status).name }}
                </button>
              ...
              <td class="px-2 py-4">
                ...
                <div
                    ...
                    role="group">
                  <Link
                      type="button" :href="route('panel.admin.page.edit',d.id)"
                  ...
                  <button
                      @click="showDialog('danger',__('delete_project?')  ,__('ok')  , edit,{cmnd:'delete-page',id:d.id ,idx:idx  })"
                      type="button" :href="route('panel.admin.page.edit',d.id)"
                      class="inline-block rounded-e  bg-danger-600 text-white px-6  py-2 text-xs font-medium uppercase leading-normal text-white transition duration-150 ease-in-out hover:bg-red-400   focus:outline-none focus:ring-0  "
                      data-te-ripple-init
                      data-te-ripple-color="light">
                    {{ __('remove') }}
                  </button>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.loading = true;
      this.data = [];
      window.axios.get(route('panel.admin.page.search'), {
        params: this.params
      }, {})
          .then((response) => {
    ...
    edit(params) {
      this.isLoading(true);
      window.axios.patch(route('panel.admin.page.update'), params,
          {})
          .then((response) => {
